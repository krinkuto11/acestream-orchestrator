...
class AceLegacyApiClient:
    ...
    def start_stream(
        ...
        # Initial catch-up bootstrap: wait for livepos, seek backwards, and
        # return the second START payload generated by LIVESEEK.
        ...
        seek_result = self.seek_stream(target_timestamp)
        seek_result["seek_target_timestamp"] = str(target_timestamp)
        seek_result["seekback"] = str(normalized_seekback)
        return seek_result

    def seek_stream(self, timestamp: int) -> Dict[str, str]:
        """Issue LIVESEEK and wait for the subsequent START response."""
        try:
        ...
        # AceStream emits a new START with updated playback URL after LIVESEEK.
        _, parts, _ = self._wait_for("START", timeout=self.response_timeout * 4)
