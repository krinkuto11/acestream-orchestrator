...
func (c *Controller) EnsureMinimum() {
	...
	metrics.DesiredReplicas.Set(float64(desired))
...
func computeDesiredReplicas(totalRunning, freeCount int, streamCounts, monitorCounts map[string]int, engines []*state.Engine) (int, string) {
	...
	st := state.Global
	threshold := cfg.MaxStreamsPerEng - 1
	var loadList []int
	for _, e := range engines {
...
func (c *Controller) reconcilerLoop(ctx context.Context) {
	defer c.wg.Done()
	ticker := time.NewTicker(config.C.AutoscaleInterval)
	defer ticker.Stop()
...
func (c *Controller) doReconcile() {
	...
	if deficit > 0 {
		...
		if createCount > 0 {
			...
			for range createCount {
				...
				spec, err := Scheduler.ScheduleNewEngine(reserved)
				if err != nil {
				...
				reserved = append(reserved, spec.ContainerName)
				id := c.nextIntentID()
				select {
				case c.intents <- intent{id: id, action: intentCreate, spec: spec}:
					metrics.IntentQueueDepth.Inc()
				default:
	...
	metrics.EnginesTotal.WithLabelValues("healthy").Set(float64(healthy))
	metrics.EnginesTotal.WithLabelValues("unhealthy").Set(float64(unhealthy))
	metrics.EnginesTotal.WithLabelValues("unknown").Set(float64(unknown))
}
...
func (c *Controller) executeIntent(ctx context.Context, it intent, consecutiveFailures *int) error {
	...
	case intentCreate:
		if !c.cb.CanProvision("general") {
			...
			metrics.ProvisioningTotal.WithLabelValues("blocked").Inc()
			return err
		}
		...
		if err != nil {
			c.cb.RecordFailure("general")
			*consecutiveFailures++
			metrics.ProvisioningTotal.WithLabelValues("failed").Inc()
			slog.Error("failed to execute create intent", "name", it.spec.ContainerName, "err", err)
			return err
		}
		c.cb.RecordSuccess("general")
		*consecutiveFailures = 0
		metrics.ProvisioningTotal.WithLabelValues("success").Inc()
