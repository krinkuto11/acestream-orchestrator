...
from ..infrastructure.docker_client import get_client, safe
...
class ResourceScheduler:
    ...
    def schedule_new_engine(self, extra_reserved_names: list[str] = None) -> Optional[EngineSpec]:
        ...
        """
        from ..infrastructure.naming import generate_container_name
    ...
    def _is_vpn_control_api_reachable(vpn_container: str, require_connected: bool = False) -> bool:
        try:
            from ..vpn.gluetun import _is_control_server_reachable_sync
    ...
    def _elect_forwarded_engine_locked(vpn_container: Optional[str]) -> tuple[bool, Optional[int]]:
        ...
        from ..services.state import state
        from ..vpn.gluetun import get_forwarded_port_sync
        from ..infrastructure.ports import alloc
        ...
        if vpn_container:
            ...
            if not has_existing and not has_pending:
                from ..vpn.gluetun import wait_for_port_sync
