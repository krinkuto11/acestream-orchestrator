...
func (p *Provisioner) ProvisionNodeForProbe(ctx context.Context, targetHostname string) (*ProvisionResult, error) {
	...
	env, err := buildGluetunEnv(provider, protocol, regions, cred, map[string]interface{}{}, false, true)
	if err != nil {
		p.creds.ReleaseLease(containerName)
		return nil, fmt.Errorf("building env: %w", err)
	}

	// Pin to the target hostname — probe must hit the exact server under test.
	if targetHostname != "" {
		env["SERVER_HOSTNAMES"] = strings.ToLower(strings.TrimSpace(targetHostname))
	}

	// Disable port forwarding for probe nodes to keep setup simple.
	env["VPN_PORT_FORWARDING"] = "off"

	labels := buildLabels(provider, protocol, lease.CredentialID, false)
	labels["acestream.probe_node"] = "true"
	...
	now := time.Now().UTC()
	assignedHostname := strings.ToLower(strings.TrimSpace(targetHostname))
	state.Global.UpsertVPNNode(&state.VPNNode{
		ContainerName:    containerName,
		ContainerID:      containerID,
		Status:           "running",
	...
	slog.Info("Probe VPN node provisioned",
		"name", containerName,
		"provider", provider,
		"hostname", assignedHostname,
	)
