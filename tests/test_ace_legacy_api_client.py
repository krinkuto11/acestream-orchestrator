...
def test_start_stream_with_seekback_uses_second_start(monkeypatch):
    client = AceLegacyApiClient("127.0.0.1", 62062)
    calls = {"seek": None}
    ...
    def fake_start(content_id, mode, stream_type="output_format=http"):
        ...
        assert mode == "infohash"
        return {
            "url": "http://127.0.0.1:6878/content/abc123/0.1",
            "stream": "1",
        }

    def fake_read_message(timeout=0):
        return (
            "EVENT",
            [
                "EVENT",
                "livepos",
                "last=1774205877",
                "live_last=1774205877",
                "last_ts=1774205877",
            ],
            {},
        )

    def fake_wait_for(expected_cmd, timeout):
        assert expected_cmd == "START"
        return (
            "START",
            ["START", "url=http://127.0.0.1:6878/content/abc123/0.2", "stream=1"],
            {},
        )
    ...
    monkeypatch.setattr(client, "_wait_for", fake_wait_for)
    monkeypatch.setattr(client, "live_seek", fake_seek)
    monkeypatch.setattr(client, "_handle_async", lambda *_: None)

    result = client.start_stream_with_seekback("abc123", mode="infohash", seekback_seconds=10)

    assert result["url"].endswith("/0.2")
    assert calls["seek"] == 1774205867


def test_start_stream_with_seekback_fallback_when_no_livepos(monkeypatch):
    client = AceLegacyApiClient("127.0.0.1", 62062)
